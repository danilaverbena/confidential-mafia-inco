 ...
 * change, a death, a public day-vote, the game's winner). Nothing here can
 * ever be a private role handle, a night-action target, or an unsettled
 * encrypted value -- those never leave the contract/client in plaintext, so
 * they physically cannot reach this type. If you find yourself wanting to
 * add a field like "role" or "nightTarget" here, stop: that value must stay
 * inside the contract until *it* reveals it (see ConfidentialMafia.sol).
 */
